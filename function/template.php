...
function zx_title()
{
    ...
    if ($zxtmp->header['title'] == '') {
        ...
        echo $zxtmp->header['title'], " - ", site_title();
    }
}
...
function out_login_link()
{
    ...
    if (is_login()) {
        ...
        echo '<a href="', site_url("login.php"), '">登入</a>';
    }
}
...
function out_Navigation_list(
    ...
    for ($i = 0; $i < $all_page; $i++) {
        echo "\t<a href=\"", str_replace("%%", $i + 1,
            $url), "\" class=\"$all_class", ($i + 1 == $now_page) ? " $now_class" : "", "\">", $i + 1, "</a>\n";
    }
    echo "</div>\n";
}
