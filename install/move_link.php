    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (POST('url') != '' && POST('url_r') != '' && POST('url') != POST('url_r')) {
            foreach ($list as $id => $v) {
                if ('' == $v['url'] || strpos($v['url'], POST('url')) === false) {
                    continue;
                }
                $list[$id]['url'] = str_replace(POST('url'), POST('url_r'), $v['url']);
                if (!in_array($id, $change)) {
                    array_push($change, $id);
                }
            }
        }
        if (POST('img') != '' && POST('img_r') != '' && POST('img') != POST('img_r')) {
            foreach ($list as $id => $v) {
                if ('' == $v['image'] || strpos($v['image'], POST('img')) === false) {
                    continue;
                }
        ...
        foreach ($change as $id) {
            echo "<li>", $id, "=>", $link->edit_link($list[$id]['id'], $list[$id]['name'], $list[$id]['url'],
                $list[$id]['category'], $list[$id]['image'], $list[$id]['flag'], $list[$id]['no'],
                $list[$id]['description']), "</li>\n";
        }
        if (count($change) > 0) {
            echo "</ul>\n";
        } else {
