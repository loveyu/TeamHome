...
class Mysql
{
    ...
    {
        $sql =
            "SELECT COUNT(*) as count FROM information_schema.TABLES WHERE TABLE_NAME='".$this->config['dbprefix']."$table' AND TABLE_SCHEMA='".$this->config['database']."'";
    ...
    {
        $sql =
            "SELECT DATE_FORMAT(  `$filed` ,  '$format' ) AS  `date` , COUNT( * ) AS `count` FROM  `".$this->config['dbprefix'].$table."` GROUP BY DATE_FORMAT(  `$filed` ,  '$format' ) ";
