...
class Link
{
    ...
    public function new_link($name, $url, $category, $image, $flag, $no, $description = '')
    {
        ...
        if (!$this->db->insert("link", [
            "name" => $name, 'url' => $url, 'image' => $image, 'category' => $category, 'description' => $description,
            'no'   => $no, 'flag' => $flag,
        ])) {
            return '创建链接失败';
        }
    ...
    public function edit_link($id, $name, $url, $category, $image, $flag, $no, $description = '')
    {
        ...
        if (!$this->db->update("link", [
            "name"        => SS($name), 'url' => SS($url), 'image' => SS($image), 'category' => SS($category),
            'description' => SS($description), 'no' => $no, 'flag' => SS($flag),
        ], "`id`=".$id)) {
            return '编辑链接失败';
        }
