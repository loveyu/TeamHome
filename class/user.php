...
class User
{
    ...
    public function logining($user = '', $password = '', $keep = '', $md5 = false)
    {
        if (is_array($user)) {
            ...
            if (!isset($user) || !isset($password) || !isset($keep) || !isset($md5)) {
                $this->login_info = [];
            } else {
    ...
    private function check_account()
    {
        ...
        if (isset($this->login_info['user'])) {
            //post登录
            $s =
                $zxdb->select("user", "id,user,username,password,active,lock",
                    "user=\"".SS($this->login_info['user'])."\" LIMIT 0 , 30");
            ...
            if (isset($this->login_info['id'])) {
                //cookie登录
                $s =
                    $zxdb->select("user", "id,user,username,password,active,lock,power",
                        "`id`=".SS($this->login_info['id'])." LIMIT 0 , 30");
    ...
    private function make_cookie()
    {
        ...
        $this->login_cookies['browser'] =
            encrypt($this->login_info['id']."\t".$this->login_info['password'], $_SERVER['HTTP_USER_AGENT']);
