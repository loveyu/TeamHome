...
class User_action
{
    ...
    public function delete_user($id)
    {
        ...
        //---删除包含该用户的其他信息
        $this->db->update("group", ['lader' => 0], "lader=".$id); //删除分组组长信息
    ...
    public function lock_user($id)
    {
        ...
        if (!$this->db->update("user", ['lock' => 'true'], "`id`=".$id)) {
            return '锁定账户失败';
        }
    ...
    public function unlock_user($id)
    {
        ...
        if (!$this->db->update("user", ['lock' => 'false'], "`id`=".$id)) {
            return '解锁账户失败';
        }
    ...
    private function make_page_info()
    {
        ...
        $this->page_info['page'] =
            ($this->page_info['number'] - $this->page_info['remainder']) / $this->page_info['one_page'];
    ...
    public function get_user_list($page)
    {
        ...
        return $this->db->select("user", "*",
            "1 LIMIT ".($page - 1) * $this->page_info['one_page'].", ".$this->page_info['one_page']);
    }
     ...
     * @return array
     */
    public function get_rand_user_list($field = [], $num = 20)
    {
        if (empty($field)) {
            $field = ['id', 'username'];
        }
    ...
    public function edit_power($id, $power)
    {
        ...
        if (!in_array($power, [0, 1, 2])) {
            return '该权限不存在';
        }
        if (!is_admin()) {
            return '非管理员操作';
        }
        if (!$this->db->update("user", ['power' => $power], "`id`=".$id)) {
            return '更新权限失败';
        }
     ...
     * @return bool
     */
    public function edit_user($id, $s = [])
    {
        //编辑用户
        if (empty($s)) {
            return false;
    ...
    public function edit_username($id, $s)
    {
        ...
        if ($this->edit_user($id, ['username' => $s])) {
            return 'OK';
        } else {
            return '更新用户名失败，但用户文件已修改，请联系管理员';
        }
    }
    ...
    public function edit_email($id, $email)
    {
        ...
        if ($this->edit_user($id, ['user' => $email])) {
            return 'OK';
        } else {
            return '更新用户名失败，请联系管理员';
        }
    }
    ...
    {
        //md5密码值 修改密码
        return $this->db->update("user", ['password' => make_password($password)], "`id`=".$id);
    }
     ...
     * @return array
     */
    public function check_edit_post($one, $old = [])
    {
    ...
    {
        //检测表单是否合法
        $forbid = ['password', 'lock', 'active', 'power', 'avata', 'verify'];
        foreach ($s as $id => $v) {
    ...
    private function up_admin_action_record($c, $id)
    {
        ...
        return $this->db->insert("admin", ['content' => $c, 'admin_id' => $id]);
    }
     ...
     * @return string
     */
    public function admin_edit_user_info(
        $name,
        $sex,
        $tel,
        $mtel,
        ...
        $edit_info =
            [
                'name'   => $name, 'sex' => $sex, 'tel' => $tel, 'mtel' => $mtel, 'qq' => $qq, 'class' => $class,
                'grade'  => $grade, 'faculty' => $faculty, 'subject' => $subject, 'group1' => $group1,
                'group2' => $group2, 'describe' => $describe, 'sign' => $sign, 'specialty' => $specialty,
            ];
        if ($this->check_post($edit_info) != 'OK') {
            return '存在异常值';
        }
        $true_edit =
            $this->check_edit_post($edit_info, $this->get_one_info($id,
                "name,tel,mtel,qq,class,grade,faculty,subject,group1,group2,describe,sign,specialty"));
        if (empty($true_edit)) {
            return '数据无需修改';
        }
    ...
    public function check_user_info($s)
    {
        ...
        if (isset($s['sex']) && $s['sex'] != '' && !in_array($s['sex'], ['男', '女'])) {
            return '性别有误';
        }
    ...
    public function user_active_username($username)
    {
        ...
        if (!$this->db->update('user', ['username' => SS($username), 'active' => 'true'],
            "`id`=".SS($this->user_info['id']))) {
            return '更新数据失败';
        }
     ...
     * @return string
     */
    public function user_edit_user_info(
        $name,
        $sex,
        $tel,
        $mtel,
