  ...
  <form action="" method="post">
      ...
      foreach ($config as $v) { ?>
        <div class="list">
          ...
          <div class="param">
              <?php $j = 0;
              foreach ($v->param as $name => $value) { ?>
                <div class="param2">
                  <label>变量名:<input name="list[<?= $i ?>][param][<?= $j ?>][name]" type="text"
                                    value="<?= $name ?>"/></label>
                  <label>变量值:<input name="list[<?= $i ?>][param][<?= $j ?>][value]" type="text"
                                    value="<?= (is_array($value)) ? implode(",", $value) : $value ?>"/></label>
                  <label>是否为数组:<input name="list[<?= $i ?>][param][<?= $j ?>][is_array]" type="checkbox"
                                      value="check" <?= (is_array($value) ? 'checked="checked" ' : "") ?>/></label>
                </div>
                  <?php $j++;
              } ?>
