...
function PCount_count_user_action(){
	if(get_cookies("login_view")){
		get_db()->insert("count",array('url'=>now_url(),'user'=>get_user_id(),'from'=>$_SERVER['HTTP_REFERER'],'ip'=>get_ip(),'ua'=>$_SERVER['HTTP_USER_AGENT']));
	}else{
		set_cookies("login_view",1);
		get_db()->insert("count",array( 'url'=>now_url(),'user'=>get_user_id(),'from'=>$_SERVER['HTTP_REFERER'],'ip'=>get_ip(),'ua'=>$_SERVER['HTTP_USER_AGENT'],'type'=>'auto_login'));
	}
}
function PCount_post_login(){
	get_db()->insert("count",array( 'url'=>now_url(),'user'=>get_user_id(),'from'=>$_SERVER['HTTP_REFERER'],'ip'=>get_ip(),'ua'=>$_SERVER['HTTP_USER_AGENT'],'type'=>'post_login'));
	set_cookies("login_view",1);
}
function PCount_logout(){
	get_db()->insert("count",array( 'url'=>now_url(),'user'=>get_user_id(),'from'=>$_SERVER['HTTP_REFERER'],'ip'=>get_ip(),'ua'=>$_SERVER['HTTP_USER_AGENT'],'type'=>'logout'));
...
function PCount_add_admin_menu(){
	...
	$zxtmp->add_admin_menu(
		'count',
		array('url'=>'plugin_page.php?m=count&id=count&p=page','name'=>'统计中心','power'=>array(0),'id'=>'count'),
		array(
			array('url'=>'plugin_page.php?m=count&id=count&p=all','name'=>'全部数据','id'=>'count_all','power'=>array(0)),
		)
	);
}
function PCount_get_serach_select(){
	...
	if(GET('user')){
		...
		foreach($tmp as $n => $v){
			if(!is_numeric($v))unset($tmp[$n]);
		}
		array_push($where,"( user in (".implode($tmp,",").") )");
	}
	if(GET('page')){
		$tmp = explode(',',GET('page'));
		foreach($tmp as $n => $v)$tmp[$n] = " ( url like '%$v%' ) ";
		array_push($where,"(".implode($tmp," OR ").")");
	}
	if(GET('time')){
		$tmp = explode(',',GET('time'));
		foreach($tmp as $n => $v)$tmp[$n] = " ( time like '%$v%' ) ";
		array_push($where,"(".implode($tmp," OR ").")");
	}
	if(GET('ua')){
		$tmp = explode(',',GET('ua'));
		foreach($tmp as $n => $v)$tmp[$n] = " ( ua like '%$v%' ) ";
		array_push($where,"(".implode($tmp," OR ").")");
	}
	if(GET('ip')){
		$tmp = explode(',',GET('ip'));
		foreach($tmp as $n => $v)$tmp[$n] = " ( ip like '%$v%' ) ";
		array_push($where,"(".implode($tmp," OR ").")");
	}
	if(GET('from')){
		$tmp = explode(',',GET('from'));
		foreach($tmp as $n => $v)$tmp[$n] = " ( form like '%$v%' ) ";
		array_push($where,"(".implode($tmp," OR ").")");
	}
	if(GET('type')){
		array_push($where,"( type = '".GET('type')."' )");
	}
	if(empty($where))$where_sql = "1";
	else $where_sql = implode($where," AND ");
