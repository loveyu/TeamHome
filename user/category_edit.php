...
$zxtmp->add_admin_menu(//增加一个菜单
    ...
    [],
    [
        ['url' => '#', 'name' => '编辑分类', 'id' => 'category_edit', 'power' => [0]],
    ]
);
$cat = $zxcat;
      ...
      <?php } else { ?>
        <form class="new" action="category_action.php" method="post">
          <table>
            <tr>
              <th>分类名称:</th>
              <td><input name="name" value="<?php echo $info['name'] ?>" type="text"/></td>
            </tr>
            <tr>
              <th>别名:</th>
              <td><input name="alias" value="<?php echo $info['alias'] ?>" type="text"/><span>必须为英文及数字</span></td>
            </tr>
            <tr valign="top">
              <th>描述:</th>
              <td><textarea name="description"><?php echo $info['description'] ?></textarea></td>
            </tr>
            <tr>
              <td colspan="2">
                <button class="submit" type="submit">编辑</button>
              </td>
            </tr>
