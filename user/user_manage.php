    ...
    <table class="list">
      ...
      <?php foreach ($user->get_user_list(GET('page')) as $v) { ?>
        <tr class="tr_<?php echo (++$i) % 2; ?>">
          ...
          <td><?php echo $v['subject'], $v['class'] ?></td>
          <td><?php
              if ($v['lock'] == 'true') {
                  echo '<a href="user_manage_unlock.php?type=unlock&id='.$v['id'].'" title="解锁账户">锁定</a> ';
              } else {
                  echo '<a href="user_manage_unlock.php?type=lock&id='.$v['id'].'" title="锁定账户">未锁定</a> ';
              }
              ...
              ?></td>
          <td><a href="user_manage_edit.php?id=<?php echo $v['id'] ?>">编辑</a> <a
              href="user_manage_delete.php?type=del&id=<?php echo $v['id'] ?>">删除</a> <a
              href="user_manage_avata.php?id=<?php echo $v['id'] ?>">修改头像</a> <a
              href="user_manage_user.php?&id=<?php echo $v['id'] ?>">修改昵称与邮箱</a></td>
        </tr>
      <?php } ?>
      <tbody>
    </table>
