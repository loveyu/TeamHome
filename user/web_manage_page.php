      ...
      <tr>
        ...
        <?php foreach ($list as $v) { ?>
          <tr class="tr_<?php echo ++$i % 2 ?>">
            <td><a href="<?php echo site_url(get_config('user_path').get_user_name().'/'.$v['name']); ?>"
                   target="_blank"><?php echo $v['name'] ?></a></td>
            <td><?php echo $file->size($v['size']) ?></td>
            <td><?php echo date("Ymd H:i:s", $v['time']); ?></td>
            <td><a href="web_edit.php?file=<?php echo $v['name'] ?>">编辑</a>&nbsp;<a
                href="web_rename.php?file=<?php echo $v['name'] ?>">重命名</a>&nbsp;<a
                href="web_edit_action.php?type=delete&path=<?php echo $v['name'] ?>">删除</a></td>
          </tr>
        <?php } ?>
        <?php if ($i == 0) {
