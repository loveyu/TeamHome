...
$zxtmp->add_admin_menu(//增加一个菜单
    ...
    [],
    [
        ['url' => '#', 'name' => '文件重命名', 'id' => 'file_rename', 'power' => [0]],
    ]
);
get_admin_header([
...
<div id="file_center">
  ...
  <form action="file_action.php" method="post">
    <ul class="creat rename">
      <li>文件路径:<input name="path" value="<?php echo GET('file') ?>" type="text"/><?php if (!$filename) {
              echo '该文件不存在或文件为目录';
          } ?></li>
      <li>新文件名:<input name="new" value="<?php if ($filename) {
              echo substr($filename, 0, strlen($filename) - 1 - strlen(pathinfo($filename, PATHINFO_EXTENSION)));
          } ?>"/>.<?php echo pathinfo($filename, PATHINFO_EXTENSION) ?><p>(文件名只允许数字,字母,和 _ )</p></li>
...
<?php if (!$filename) { ?>
  <script>alert('文件 <?php echo GET('file')?> 不存在，或有误，或为路径')</script><?php } ?>
