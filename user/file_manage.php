...
?>
  <div id="file_center">
    ...
    <form class="creat" action="file_action.php" method="post">
      ...
      <input class="input" name="name" value="" type="text"/>.
      <select name="ext">
          <?php foreach ($file->text_array() as $v) {
    ...
    <table class="file_m">
      ...
      <tr class="last_path">
        <td colspan="3"><a href="?dir=<?php echo str_replace('\\', '/', dirname(GET('dir'))) ?>">上级目录</a></td>
      </tr>

        <?php foreach ($list['dir'] as $v) { ?>
          <tr class="tr_<?php echo ++$i % 2 ?>">
            <td>[D]&nbsp;<a href="?dir=<?php echo urlencode(GET('dir').$v) ?>"><?php echo $v; ?></a></td>
            <td></td>
            <td><a href="file_action.php?type=delete&path=<?php echo GET('dir').'/'.$v ?>">删除</a></td>
          </tr>
        <?php }
        foreach ($list['file'] as $v) {
          ...
          <tr class="tr_<?php echo ++$i % 2 ?>">
            <td>[F]&nbsp;<a
                href="<?php echo site_url(str_replace("//", '/', get_config('attach_path').GET('dir').$v)) ?>"
                target="_blank"><?php echo $v; ?></a></td>
            <td><?php if ($file->is_text($v)) {
                    echo '<a href="file_edit.php?file=', urlencode(GET('dir').$v), '">编辑</a>&nbsp;';
                } ?><a href="file_rename.php?file=<?php echo urlencode(GET('dir').$v) ?>&">重命名</a></td>
            <td><a href="file_action.php?path=<?php echo urlencode(GET('dir').$v) ?>&type=delete">删除</a></td>
          </tr>
        <?php }
        if ($i == 0) {
            echo "<tr class=\"no_file\"><td colspan=\"3\">没有文件或文件夹被创建</td></tr>\n";
        } ?>
    </table>
