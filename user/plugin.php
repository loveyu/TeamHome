        ...
        <tr>
          ...
          <?php foreach ($zxplugin->m_get_file_list() as $name) { ?>
            <tr class="tr_<?php echo $i % 2; ?>">
              <td><?php echo $name ?><?php if (isset($sql_plugin[$name])) {
                      echo "&nbsp;&nbsp;启动类型:", $sql_plugin[$name];
                  } ?></td>
                <?php if (!isset($sql_plugin[$name])) { ?>
                  <td>
                    <form method="get" action="plugin_action.php">类型:<select name="type"><?php
                            foreach ($zxplugin->plugin_type() as $v) {
                ...
                <?php } ?>
              <td><a href="plugin_action.php?name=<?php echo $name ?>&action=delete">删除</a></td>
            </tr>
          <?php } ?>
      </table>
    </div>
      <?php if (!empty($tmp_plugin)) {
          $zxplugin->clear_invalid_plugin($tmp_plugin); ?>
        <div class="notice">
