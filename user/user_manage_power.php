...
if(is_post() && iPOST('power')){
	...
	if($err=='OK')redirect(site_url('user/user_manage_power.php?status=OK&id=').POST('id'));
}
...
if($id!='')$info = $user->get_one_info($id,"power,name");
//print_r($info);
if(isset($info['power']))$power = $info['power'];
else $power = POST('power');
?>
<div id="user_manage">
<h3>修改用户权限</h3>
<?php
if($err)echo '<p class="notice">',$err,'</p>';
if(GET('status')=='OK')echo '<p class="notice">权限已修改</p>';
?>
<form class="power" action="user_manage_power.php" method="post">
<ul>
<li><label>操作ID:<input name="id" value="<?php echo $id?>" type="text" /><?php if(isset($info['name']))echo $info['name'];?></label></li>
<li><label>权限等级:<select name="power">
<?php foreach($zxuser->get_power_group() as $id => $v){?>
<option value="<?php echo $id?>"<?php if($power==$id)echo " selected"?>><?php echo $v?></option>
<?php }?>
</select></label></li>
