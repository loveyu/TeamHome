  ...
  <div id="new_center">
    ...
    <table class="list">
      <tr>
        ...
        <th>分类</th>
        <th>类型</th>
        <th>允许评论</th><?php if (is_admin()) echo '<th>发布者</th>' ?>
        <th>操作</th>
      </tr>
        <?php foreach ($list as $v) { ?>
          <tr class="tr_<?php echo (++$i) % 2; ?>">
            <td><?php echo $v['id'] ?></td>
            <td><a href="<?php echo site_url('view.php?id='.$v['id']) ?>"><?php echo $v['title'] ?></a></td>
            <td><?php echo $v['author'] ?></td>
            <td><?php echo $v['time'] ?></td>
            <td><?php echo $news->get_category_list($v['category']) ?></td>
            <td><?php echo $v['type'] ?></td>
            <td><?php echo $v['comment'] ?></td>
              <?php if (is_admin()) {
                  echo "<td>", $v['user'], "</td>";
              } ?>
            <td><a href="news_edit.php?id=<?php echo $v['id'] ?>">编辑</a><?php if (is_admin()) { ?><a
                href="news_post_edit.php?action=delete&id=<?php echo $v['id'] ?>">&nbsp;删除</a><?php } ?></td>
          </tr>
        <?php }
        if ($i == 0) {
            echo '<tr class="no_category"><td colspan="', is_admin() ? '9' : '8', '">没有发现内容</td></tr>';
        } ?>
    </table>
  </div>
