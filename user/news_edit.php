...
$zxtmp->add_admin_menu(//增加一个菜单
    ...
    [],
    [
        ['url' => '#', 'name' => '编辑新闻', 'id' => 'news_edit', 'power' => [0, 1]],
    ]
);
  ...
  <div id="new_center">
    ...
    <form method="post" action="news_post_edit.php">
      ...
      <div class="type">
        <label>类型:<select name="category">
                <?php foreach ($news->get_category_list() as $v1_id => $v1) { ?>
                  <option
                    value="<?php echo $v1_id ?>"<?php if ($info['category'] == $v1_id) echo ' selected' ?>><?php echo $v1 ?></option>
                <?php } ?>
          </select></label>
        ...
        <label>允许评论:<select name="comment">
            <option value="false"<?php if ($info['comment'] == 'false') {
                echo ' selected';
            } ?>>禁止
            </option>
            <option value="true"<?php if ($info['comment'] == 'true') {
                echo ' selected';
            } ?>>允许
            </option>
          </select>
          <label>状态:<select name="type">
              <option value="publish"<?php if ($info['type'] == 'publish') {
                  echo ' selected';
              } ?>>发布
              </option>
              <option value="draft"<?php if ($info['type'] == 'draft') {
                  echo ' selected';
              } ?>>草稿
              </option>
              <option value="hidden"<?php if ($info['type'] == 'hidden') {
                  echo ' selected';
              } ?>>隐藏
              </option>
            </select></label>
      ...
      <textarea name="content"
                style="width:99.8%;height:600px;visibility:hidden;"><?php echo htmlspecialchars($info['content']) ?></textarea>
