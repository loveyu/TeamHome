...
$zxtmp->set_page_id('file', 'edit');
$zxtmp->add_admin_menu(//增加一个菜单
    'file-center',
    [],
    [
        ['url' => '#', 'name' => '编辑页面', 'id' => 'file_edit', 'power' => [0]],
    ]
);
load('file_action');
...
if ($file->is_attach_file(GET('file'), true)) {
    $file_content = $file->get_attach_file(GET('file'));
...
?>
  <div id="file_center">
      <?php if (isset($file_content)) { ?>
        <form class="edit_charset" action="file_edit.php" method="get"><!-- 编码切换 -->
          <input type="hidden" name="file" value="<?php if (isset($file_content)) echo GET('file') ?>"/>
          <select name="charset">
            <option value="UTF-8"<?php if (GET('charset') == 'UTF-8') {
                echo ' selected="selected"';
            } ?>>UTF-8
            </option>
            <option value="GB2312"<?php if (GET('charset') == 'GB2312') {
                echo ' selected="selected"';
            } ?>>GB2312
            </option>
            <option value="GBK"<?php if (GET('charset') == 'GBK') {
                echo ' selected="selected"';
            } ?>>GBK
            </option>
          </select>
          <button type="submit">切换编码</button>
        </form>

        <form class="edit" action="file_action.php" method="post"><!-- 编辑框 -->
          <p class="file_info">
            当前文件:<span><?php echo basename(GET('file')) ?></span>路径:<span><?php echo GET('file') ?></span>
            <button class="submit" type="submit">保存</button>
          </p>
          <textarea name="content" style="width:99.9%;height:400px;visibility:hidden;"><?php if (isset($file_content)) {
                  echo htmlspecialchars($file_content, ENT_QUOTES);
              } ?></textarea>
          <input type="hidden" name="file" value="<?php if (isset($file_content)) echo GET('file') ?>"/>
