...
if(is_post()){
	if(iPOST('id') && POST('id')==get_user_id() && iFILES('avata')){
		...
		if($err=='OK')redirect(site_url("user/edit_img.php?up=OK"),'refresh');
	}else $err = '提交的数据有误';
}
...
<ul>
<li><img src="<?php echo get_avata($user->user_info['id']);?>" height="64" width="64" alt="img" /><p>当前头像</p></li>
<li><input type="file" name="avata" /><p>图片将默认压缩到64*64,仅允许上传`jpg`,`gif`,`png`格式</p></li>
<li><button type="submit">上传图片</button></li>
</ul>
