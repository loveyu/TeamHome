...
$zxtmp->add_admin_menu(//增加一个菜单
    ...
    [],
    [
        ['url' => '#', 'name' => '删除分类', 'id' => 'category_delete', 'power' => [0]],
    ]
);
$cat = $zxcat;
...
if (!empty($info)) {
    $cat_info = $cat->get_category($info['type']);
}
    ...
    <h3>删除某一分类</h3>
      ...
      <?php } else { ?>
        ...
        <tr>
          ...
          <tr>
            <th>移动分类到：</th>
            <td><select name="move"><?php foreach ($cat_info as $v) {
                        if ($v['id'] != $info['id']) { ?>
          ...
          <tr>
            <td colspan="2">
              <button class="submit" type="submit">删除</button>
            </td>
          </tr>
          </table>
          </form>
