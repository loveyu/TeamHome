...
?>
  <div id="file_center">
    ...
    <form class="edit" action="file_style_action.php" method="post"><!-- 编辑框 -->
      <p class="file_info">当前文件:<span><?php echo $tfile ?>.css</span>主题:<select id="theme_select"
                                                                                name="theme"><?php foreach ($zxsys->get_style_list() as $v) { ?>
      ...
      <p class="file_info">
          ...
          foreach ($file->get_style_theme_css_list($theme) as $v) {
              echo "<a href=\"file_style.php?theme=$theme&file=", substr($v['name'], 0,
                  -4), "\">", $v['name'], "</a>&nbsp;&nbsp;";
          }
          ?>
      </p>
      <textarea name="content"
                style="width:99.9%;height:400px;visibility:hidden;"><?php echo htmlspecialchars($style_content,
              ENT_QUOTES); ?></textarea>
