    ...
    <h3>删除分组</h3>
      <?php if (isset($group->group[GET('id')])) { ?>
        <form action="group_action.php" method="post">
          <ul class="delete">
            <li>要删除的分组:<input name="id" value="<?php echo GET('id') ?>"
                              type="hidden"/><?php echo $group->group[GET('id')]['name'] ?>
              (<?php echo $group->group[GET('id')]['id'] ?>)
            </li>
              <?php if ($group->group[GET('id')]['parent'] == 0) { ?>
                <li>移动用户分组到:<select name="move_user">
                      <?php foreach ($group->group as $v) {
                          if ($v['parent'] == 0 && $v['id'] != GET('id')) { ?>
              ...
              <?php } ?>
            <li>
              <button class="submit" type="submit">删除分组</button>
            </li>
          </ul>
