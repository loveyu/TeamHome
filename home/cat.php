<?php
	...
	get_header(array('title' => '文章分类 - ' . $cat));
?>
<div class="content_resize">
	<div class="mainbar">
		...
		<div class="article">
			...
			<p class="tag">编辑者：<span><?php echo $v['author']?></span> | 分类:<span><a href="<?php echo site_url()?>cat/<?php echo $v['category']?>" ><?php echo $cat ?></a></span></p>
			<?php echo Theme_get_text_content($v['content'], 150)?>
			<p class="tag">发布时间:<span><?php echo Theme_format_time($v['time'])?></span></p>
		</div>
		<?php }?>
		<?php out_Navigation_list($news->page_info['all_page'], GET('p'), site_url() . "cat/$id?p=%%" . ((GET('id')) ? '&id=' . GET('id') : ''), "list", "now", "nav_list", 'info_nav'); ?>
	</div>
